...
### 6.1 로컬 락 vs 비관적 락 개념 실험
- `/ui/lock-concept-demo` API를 호출하는 버튼을 통해,
  인스턴스별 @Synchronized(로컬 락)과 전역 공유 락(비관적 락 개념)의 차이를
  초기 재고 1개, 2개의 "서버 인스턴스"가 동시에 주문하는 상황으로 비교할 수 있습니다.
